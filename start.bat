...
`Executables\busybox wget -q --spider --server-response https://raw.githubusercontent.com/Ajatt-Tools/chmonime/master/chmonime.sh -O - 2^>^&1 ^| Executables\busybox sed -ne "/Content-Length/{s/.*: //;p}"`) DO (
SET remote=%%F
)
