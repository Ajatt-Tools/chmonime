...
if test "$episode"; then
    ...
    choose=$(echo "$animelist" | grep -o "name\"\].*\|size\"\].*" | awk '{getline x;print x;}1' | awk 'NR%2 {printf "%s ",$0;next;}1' | sed 's/size"]//g;s/name"]//g;s/"//g;s/\t//g;s/ / | /' | sort -u | sort -t'|' -k2)
    if test "$quality"; then
        choose1=$(echo "$choose" | grep "$quality")
        choose2=$(echo "$choose" | grep -v "$quality")
        choose=$(echo -e "$choose1" "\n" "$choose2" | sed -e 's/^[ \t]*//' | sed '/^[[:space:]]*$/d' | fzf -m --reverse --no-sort --exact)
...
if [[ "$autoplay" == "yes" && "$episode" -gt "0" ]]; then
    if uname | grep -i -q "Windows\|Mingw\|Cygwin"; then
        ...
        while IFS= read -r line; do
            ...
            echo "cmd /C \"start /B xdccget.exe --dont-confirm-offsets -d \"$foldir\" -q \"irc.rizon.net\" \"#nibl\" \"$botname xdcc send #$pacname\" & start /B timeout 10 >nul && when_changed.exe \"$foldir\\\" $player %file% \"" >>"$2"
        done <"$1"
    else
