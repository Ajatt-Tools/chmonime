...
if [[ "$autoplay" == "yes" && "$episode" -gt "0" ]]; then
    if uname | grep -i -q "Windows\|Mingw\|Cygwin"; then
        ...
        while IFS= read -r line; do
            ...
            echo "cmd /C \"start /B xdccget.exe --dont-confirm-offsets -d \"$foldir\" -q \"irc.rizon.net\" \"#nibl\" \"$botname xdcc send #$pacname\" & start /B when_changed.exe \"$foldir\" $player %file% >nul\"" >>"$2"
        done <"$1"
    else
